...
/*   Updated: 2025/06/30 18:31:45 by linliu           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
mlx_image_to_window(mlx, img, x, y); Step 4: Put the image into your game window
*/


mlx_image_t	*load_textre(t_game *game)
{
	mlx_texture_t *tmp;
	mlx_image_t	*img;


	tmp = mlx_load_png("texture/player.png");
	if (!tmp)
	...
	{
		mlx_delete_texture(tmp);
		terminate_with_error(game, "fail to convert texture\n");
	}
	mlx_delete_texture(tmp);
	return (img);
}
